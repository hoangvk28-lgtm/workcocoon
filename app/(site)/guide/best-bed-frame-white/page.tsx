...
import {
  ...
  buyingCriteria,
  type GuideProduct,
} from "@/data/guides/best-bed-frame-white";
...
export const metadata: Metadata = buildMetadata({
  ...
  path: "/guide/best-bed-frame-white",
...
export default async function Page() {
  ...
  const articleSchema = {
    ...
    mainEntityOfPage: { "@type": "WebPage", "@id": `${SITE_URL}/guide/best-bed-frame-white` },
    about: [{"@type":"Thing","name":"Best White Bed Frames"}],
  };
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 3, name: "Best White Bed Frames", item: `${SITE_URL}/guide/best-bed-frame-white` },
    ],
  };
  ...
  const itemListSchema = {
    ...
    itemListElement: products.map((p) => ({
      ...
      position: p.rank,
      name: `${p.name} - ${p.badge}`,
      url: `${SITE_URL}/guide/best-bed-frame-white#${p.id}`,
    })),
  };
    ...
    <>
      ...
      <Container className="py-12 max-w-4xl">
        <nav className="flex items-center gap-1.5 text-xs text-ink-muted mb-6">
          ...
          <span>/</span>
          <span className="text-ink">Best White Bed Frames</span>
        </nav>
        ...
        <div className="rounded-2xl overflow-hidden border border-border mb-10 bg-bg">
          <Image src={heroImg} alt="Best White Bed Frames" width={900} height={420} className="w-full h-auto object-contain" priority unoptimized />
        </div>
        ...
        <section className="mb-10 space-y-4 text-base text-ink-secondary leading-relaxed">
          <p>White bed frames brighten up small bedrooms and pair with almost any color scheme, but the category spans everything from plain powder-coated metal platforms to upholstered velvet wingback headboards with built-in storage.</p>
          <p>Below are 8 white bed frames we evaluated on style, weight capacity, and extra features like drawers and charging ports, ranging from a $49.97 minimalist metal platform to a fully-loaded velvet frame with LED shelving and USB charging.</p>
        </section>
        ...
        <section className="mb-14 p-6 rounded-2xl flex flex-col items-center text-center" style={{ background: "linear-gradient(135deg, #FF9900 0%, #e68900 100%)" }}>
          ...
          <a href={`https://www.amazon.com/s?k=bed+frame+white&tag=deskfinds0d-20`} target="_blank" rel="noopener noreferrer sponsored" className="inline-flex items-center gap-2 px-6 py-3 rounded-xl bg-white font-bold text-sm hover:bg-gray-50 transition-colors" style={{ color: "#FF9900" }}>
            Shop on Amazon &rarr;
          </a>
        </section>
