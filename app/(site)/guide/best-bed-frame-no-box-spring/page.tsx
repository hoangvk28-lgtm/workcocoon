...
import {
  ...
  buyingCriteria,
  type GuideProduct,
} from "@/data/guides/best-bed-frame-no-box-spring";
...
export const metadata: Metadata = buildMetadata({
  ...
  path: "/guide/best-bed-frame-no-box-spring",
...
export default async function Page() {
  ...
  const articleSchema = {
    ...
    mainEntityOfPage: { "@type": "WebPage", "@id": `${SITE_URL}/guide/best-bed-frame-no-box-spring` },
    about: [{"@type":"Thing","name":"Best No-Box-Spring Bed Frames"}],
  };
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 3, name: "Best No-Box-Spring Bed Frames", item: `${SITE_URL}/guide/best-bed-frame-no-box-spring` },
    ],
  };
  ...
  const itemListSchema = {
    ...
    itemListElement: products.map((p) => ({
      ...
      position: p.rank,
      name: `${p.name} - ${p.badge}`,
      url: `${SITE_URL}/guide/best-bed-frame-no-box-spring#${p.id}`,
    })),
  };
    ...
    <>
      ...
      <Container className="py-12 max-w-4xl">
        <nav className="flex items-center gap-1.5 text-xs text-ink-muted mb-6">
          ...
          <span>/</span>
          <span className="text-ink">Best No-Box-Spring Bed Frames</span>
        </nav>
        ...
        <div className="rounded-2xl overflow-hidden border border-border mb-10 bg-bg">
          <Image src={heroImg} alt="Best No-Box-Spring Bed Frames" width={900} height={420} className="w-full h-auto object-contain" priority unoptimized />
        </div>
        ...
        <section className="mb-10 space-y-4 text-base text-ink-secondary leading-relaxed">
          <p>Ditching the box spring is one of the easiest ways to save money and floor-to-mattress height, but it only works if the frame underneath has slats or a solid deck strong enough to support the mattress on its own.</p>
          <p>Below are 8 bed frames we evaluated on weight capacity, underbed clearance, and noise-free construction, all built to hold a mattress directly without a separate foundation, ranging from $44.96 tool-free metal folders to a 1,000 lb rated solid pine platform.</p>
        </section>
        ...
        <section className="mb-14 p-6 rounded-2xl flex flex-col items-center text-center" style={{ background: "linear-gradient(135deg, #FF9900 0%, #e68900 100%)" }}>
          ...
          <a href={`https://www.amazon.com/s?k=bed+frame+no+box+spring+needed&tag=deskfinds0d-20`} target="_blank" rel="noopener noreferrer sponsored" className="inline-flex items-center gap-2 px-6 py-3 rounded-xl bg-white font-bold text-sm hover:bg-gray-50 transition-colors" style={{ color: "#FF9900" }}>
            Shop on Amazon &rarr;
          </a>
        </section>
