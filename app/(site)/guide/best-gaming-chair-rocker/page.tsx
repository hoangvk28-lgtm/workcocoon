...
import {
  ...
  buyingCriteria,
  type GuideProduct,
} from "@/data/guides/best-gaming-chair-rocker";
...
export const metadata: Metadata = buildMetadata({
  ...
  path: "/guide/best-gaming-chair-rocker",
...
export default async function Page() {
  ...
  const articleSchema = {
    ...
    mainEntityOfPage: { "@type": "WebPage", "@id": `${SITE_URL}/guide/best-gaming-chair-rocker` },
    about: [{"@type":"Thing","name":"Best Gaming Chair Rockers"}],
  };
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 3, name: "Best Gaming Chair Rockers", item: `${SITE_URL}/guide/best-gaming-chair-rocker` },
    ],
  };
  ...
  const itemListSchema = {
    ...
    itemListElement: products.map((p) => ({
      ...
      position: p.rank,
      name: `${p.name} - ${p.badge}`,
      url: `${SITE_URL}/guide/best-gaming-chair-rocker#${p.id}`,
    })),
  };
    ...
    <>
      ...
      <Container className="py-12 max-w-4xl">
        <nav className="flex items-center gap-1.5 text-xs text-ink-muted mb-6">
          ...
          <span>/</span>
          <span className="text-ink">Best Gaming Chair Rockers</span>
        </nav>
        ...
        <div className="rounded-2xl overflow-hidden border border-border mb-10 bg-bg">
          <Image src={heroImg} alt="Best Gaming Chair Rockers" width={900} height={420} className="w-full h-auto object-contain" priority unoptimized />
        </div>
        ...
        <section className="mb-10 space-y-4 text-base text-ink-secondary leading-relaxed">
          <p>A gaming chair rocker gives you back-and-forth motion instead of a rigid seat, and the mechanism varies a lot between designs: some rock from a low floor-level base, others pivot on a pedestal, and a few combine rocking with a full recliner.</p>
          <p>We evaluated eight gaming chair rockers based on swivel range, rocking degree, and overall design, from Amazon's best-selling floor rocker to Bluetooth speaker models and full recliner-rockers.</p>
        </section>
        ...
        <section className="mb-14 p-6 rounded-2xl flex flex-col items-center text-center" style={{ background: "linear-gradient(135deg, #FF9900 0%, #e68900 100%)" }}>
          ...
          <a href={`https://www.amazon.com/s?k=gaming+chair+rocker&tag=deskfinds0d-20`} target="_blank" rel="noopener noreferrer sponsored" className="inline-flex items-center gap-2 px-6 py-3 rounded-xl bg-white font-bold text-sm hover:bg-gray-50 transition-colors" style={{ color: "#FF9900" }}>
            Shop on Amazon &rarr;
          </a>
        </section>
