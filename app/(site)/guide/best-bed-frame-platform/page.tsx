...
import {
  ...
  buyingCriteria,
  type GuideProduct,
} from "@/data/guides/best-bed-frame-platform";
...
export const metadata: Metadata = buildMetadata({
  ...
  path: "/guide/best-bed-frame-platform",
...
export default async function Page() {
  ...
  const articleSchema = {
    ...
    mainEntityOfPage: { "@type": "WebPage", "@id": `${SITE_URL}/guide/best-bed-frame-platform` },
    about: [{"@type":"Thing","name":"Best Platform Bed Frames"}],
  };
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 3, name: "Best Platform Bed Frames", item: `${SITE_URL}/guide/best-bed-frame-platform` },
    ],
  };
  ...
  const itemListSchema = {
    ...
    itemListElement: products.map((p) => ({
      ...
      position: p.rank,
      name: `${p.name} - ${p.badge}`,
      url: `${SITE_URL}/guide/best-bed-frame-platform#${p.id}`,
    })),
  };
    ...
    <>
      ...
      <Container className="py-12 max-w-4xl">
        <nav className="flex items-center gap-1.5 text-xs text-ink-muted mb-6">
          ...
          <span>/</span>
          <span className="text-ink">Best Platform Bed Frames</span>
        </nav>
        ...
        <div className="rounded-2xl overflow-hidden border border-border mb-10 bg-bg">
          <Image src={heroImg} alt="Best Platform Bed Frames" width={900} height={420} className="w-full h-auto object-contain" priority unoptimized />
        </div>
        ...
        <section className="mb-10 space-y-4 text-base text-ink-secondary leading-relaxed">
          <p>Platform beds have become the default choice for anyone who wants to skip the box spring, and 2026's lineup ranges from simple folding metal frames under $50 to wood-and-metal combos with built-in headboards and LED lighting.</p>
          <p>Below are 8 platform bed frames we evaluated on weight capacity, underbed clearance, assembly difficulty, and build quality, all sized for Queen mattresses unless noted, so you can match the frame to your room and budget.</p>
        </section>
        ...
        <section className="mb-14 p-6 rounded-2xl flex flex-col items-center text-center" style={{ background: "linear-gradient(135deg, #FF9900 0%, #e68900 100%)" }}>
          ...
          <a href={`https://www.amazon.com/s?k=platform+bed+frame&tag=deskfinds0d-20`} target="_blank" rel="noopener noreferrer sponsored" className="inline-flex items-center gap-2 px-6 py-3 rounded-xl bg-white font-bold text-sm hover:bg-gray-50 transition-colors" style={{ color: "#FF9900" }}>
            Shop on Amazon &rarr;
          </a>
        </section>
