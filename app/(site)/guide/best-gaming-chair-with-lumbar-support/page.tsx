...
import {
  ...
  buyingCriteria,
  type GuideProduct,
} from "@/data/guides/best-gaming-chair-with-lumbar-support";
...
export const metadata: Metadata = buildMetadata({
  ...
  path: "/guide/best-gaming-chair-with-lumbar-support",
...
export default async function Page() {
  ...
  const articleSchema = {
    ...
    mainEntityOfPage: { "@type": "WebPage", "@id": `${SITE_URL}/guide/best-gaming-chair-with-lumbar-support` },
    about: [{"@type":"Thing","name":"Best Gaming Chairs with Lumbar Support"}],
  };
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 3, name: "Best Gaming Chairs with Lumbar Support", item: `${SITE_URL}/guide/best-gaming-chair-with-lumbar-support` },
    ],
  };
  ...
  const itemListSchema = {
    ...
    itemListElement: products.map((p) => ({
      ...
      position: p.rank,
      name: `${p.name} - ${p.badge}`,
      url: `${SITE_URL}/guide/best-gaming-chair-with-lumbar-support#${p.id}`,
    })),
  };
    ...
    <>
      ...
      <Container className="py-12 max-w-4xl">
        <nav className="flex items-center gap-1.5 text-xs text-ink-muted mb-6">
          ...
          <span>/</span>
          <span className="text-ink">Best Gaming Chairs with Lumbar Support</span>
        </nav>
        ...
        <div className="rounded-2xl overflow-hidden border border-border mb-10 bg-bg">
          <Image src={heroImg} alt="Best Gaming Chairs with Lumbar Support" width={900} height={420} className="w-full h-auto object-contain" priority unoptimized />
        </div>
        ...
        <section className="mb-10 space-y-4 text-base text-ink-secondary leading-relaxed">
          <p>Lumbar support on a gaming chair ranges from a simple removable pillow strapped to the backrest to a micro-adjustable dial system that targets a precise point on your lower spine, and the right choice depends on how much customization you actually need.</p>
          <p>We evaluated these eight lumbar-focused gaming chairs based on product specs and buyer feedback, ranking them by adjustability, review volume, and how well the lumbar design matches different body types and budgets.</p>
        </section>
        ...
        <section className="mb-14 p-6 rounded-2xl flex flex-col items-center text-center" style={{ background: "linear-gradient(135deg, #FF9900 0%, #e68900 100%)" }}>
          ...
          <a href={`https://www.amazon.com/s?k=gaming+chair+with+lumbar+support&tag=deskfinds0d-20`} target="_blank" rel="noopener noreferrer sponsored" className="inline-flex items-center gap-2 px-6 py-3 rounded-xl bg-white font-bold text-sm hover:bg-gray-50 transition-colors" style={{ color: "#FF9900" }}>
            Shop on Amazon &rarr;
          </a>
        </section>
