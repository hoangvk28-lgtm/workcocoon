...
import {
  ...
  buyingCriteria,
  type GuideProduct,
} from "@/data/guides/best-bed-frame-risers";
...
export const metadata: Metadata = buildMetadata({
  ...
  path: "/guide/best-bed-frame-risers",
...
export default async function Page() {
  ...
  const articleSchema = {
    ...
    mainEntityOfPage: { "@type": "WebPage", "@id": `${SITE_URL}/guide/best-bed-frame-risers` },
    about: [{"@type":"Thing","name":"Best Bed Frame Risers"}],
  };
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 3, name: "Best Bed Frame Risers", item: `${SITE_URL}/guide/best-bed-frame-risers` },
    ],
  };
  ...
  const itemListSchema = {
    ...
    itemListElement: products.map((p) => ({
      ...
      position: p.rank,
      name: `${p.name} - ${p.badge}`,
      url: `${SITE_URL}/guide/best-bed-frame-risers#${p.id}`,
    })),
  };
    ...
    <>
      ...
      <Container className="py-12 max-w-4xl">
        <nav className="flex items-center gap-1.5 text-xs text-ink-muted mb-6">
          ...
          <span>/</span>
          <span className="text-ink">Best Bed Frame Risers</span>
        </nav>
        ...
        <div className="rounded-2xl overflow-hidden border border-border mb-10 bg-bg">
          <Image src={heroImg} alt="Best Bed Frame Risers" width={900} height={420} className="w-full h-auto object-contain" priority unoptimized />
        </div>
        ...
        <section className="mb-10 space-y-4 text-base text-ink-secondary leading-relaxed">
          <p>Bed frame risers are one of the cheapest ways to add real underbed storage clearance without buying a new bed, raising an existing frame, sofa, or desk by anywhere from 1 to 8 inches depending on the set.</p>
          <p>Below are 8 bed frame risers we evaluated on weight capacity per riser, stacking height options, and how securely they grip different leg shapes, so you can find the right set whether you need a couple extra inches or a full storage bin's worth of clearance.</p>
        </section>
        ...
        <section className="mb-14 p-6 rounded-2xl flex flex-col items-center text-center" style={{ background: "linear-gradient(135deg, #FF9900 0%, #e68900 100%)" }}>
          ...
          <a href={`https://www.amazon.com/s?k=bed+frame+risers&tag=deskfinds0d-20`} target="_blank" rel="noopener noreferrer sponsored" className="inline-flex items-center gap-2 px-6 py-3 rounded-xl bg-white font-bold text-sm hover:bg-gray-50 transition-colors" style={{ color: "#FF9900" }}>
            Shop on Amazon &rarr;
          </a>
        </section>
