...
import {
  ...
  buyingCriteria,
  type GuideProduct,
} from "@/data/guides/best-bookshelf-nightstand";
...
export const metadata: Metadata = buildMetadata({
  ...
  path: "/guide/best-bookshelf-nightstand",
...
export default async function Page() {
  ...
  const articleSchema = {
    ...
    mainEntityOfPage: { "@type": "WebPage", "@id": `${SITE_URL}/guide/best-bookshelf-nightstand` },
    about: [{"@type":"Thing","name":"Best Bookshelf Nightstands"}],
  };
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 3, name: "Best Bookshelf Nightstands", item: `${SITE_URL}/guide/best-bookshelf-nightstand` },
    ],
  };
  ...
  const itemListSchema = {
    ...
    itemListElement: products.map((p) => ({
      ...
      position: p.rank,
      name: `${p.name} - ${p.badge}`,
      url: `${SITE_URL}/guide/best-bookshelf-nightstand#${p.id}`,
    })),
  };
    ...
    <>
      ...
      <Container className="py-12 max-w-4xl">
        <nav className="flex items-center gap-1.5 text-xs text-ink-muted mb-6">
          ...
          <span>/</span>
          <span className="text-ink">Best Bookshelf Nightstands</span>
        </nav>
        ...
        <div className="rounded-2xl overflow-hidden border border-border mb-10 bg-bg">
          <Image src={heroImg} alt="Best Bookshelf Nightstands" width={900} height={420} className="w-full h-auto object-contain" priority unoptimized />
        </div>
        ...
        <section className="mb-10 space-y-4 text-base text-ink-secondary leading-relaxed">
          <p>Bookshelf nightstands generally fall into two groups: simple 3-tier open shelving that maximizes visible storage in a compact footprint, and taller nightstands that combine open bookshelf tiers with drawers, LED lighting, and built-in charging for maximizing vertical storage in a smaller bedroom.</p>
          <p>We evaluated 8 bookshelf nightstands on shelf count, drawer storage, height, and extras like charging ports and LED lighting, ranging from budget-friendly open shelves to feature-packed tall towers.</p>
        </section>
        ...
        <section className="mb-14 p-6 rounded-2xl flex flex-col items-center text-center" style={{ background: "linear-gradient(135deg, #FF9900 0%, #e68900 100%)" }}>
          ...
          <a href={`https://www.amazon.com/s?k=bookshelf+nightstand&tag=deskfinds0d-20`} target="_blank" rel="noopener noreferrer sponsored" className="inline-flex items-center gap-2 px-6 py-3 rounded-xl bg-white font-bold text-sm hover:bg-gray-50 transition-colors" style={{ color: "#FF9900" }}>
            Shop on Amazon &rarr;
          </a>
        </section>
